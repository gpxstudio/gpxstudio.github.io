...
export default class TileBelt {
    ...
    getSiblings(tile) {
        return this.getChildren(getParent(tile));
    }
    ...
    hasSiblings(tile, tiles) {
        var siblings = this.getSiblings(tile);
    ...
    bboxToTile(bboxCoords) {
        var min = this.pointToTile(bboxCoords[0], bboxCoords[1], 32);
        var max = this.pointToTile(bboxCoords[2], bboxCoords[3], 32);
        var bbox = [min[0], min[1], max[0], max[1]];

        var z = this.getBboxZoom(bbox);
        if (z === 0) return [0, 0, 0];
        var x = bbox[0] >>> (32 - z);
        var y = bbox[1] >>> (32 - z);
        return [x, y, z];
